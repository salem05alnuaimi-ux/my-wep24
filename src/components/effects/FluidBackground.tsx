...
vec3 encodeFluid(vec3 state) { return state / 0.8 + 0.5; }
vec3 decodeFluid(vec3 raw)   { return (raw - 0.5) * 0.8; }
...
void main() {
  ...
  if (motionLen > 6.0) motionVec = motionVec / motionLen * 6.0;

  bool hasMotion = length(iMouse.xy) > 0.0;
  if (hasMotion) {
    float q = mix(lineDistance(pixel, mousePos, mousePrev), distance(pixel, mousePos), 0.4);
    float brushMask = exp(-q * q * (2.2e-4 / uBrushSize));
    vel += motionVec * brushMask * 0.03 * uBrushStrength;
    ink += brushMask * 0.03 * uBrushStrength * 3.0;
  }
  ...
  if (hasMotion && motionLen < 0.5) {
    float nearCursor = exp(-distance(pixel, mousePos) * distance(pixel, mousePos) * 1e-5);
...
float fbm(vec2 p) {
  ...
  for (int i = 0; i < 6; i++) { s += noise(p * fr) * a; a *= 0.5; fr *= 2.0; }
  return s;
}

vec3 sampleFluidState(vec2 uv) { return (texture2D(u_fluid, uv).rgb - 0.5) * 0.8; }

void main() {
  vec2 uv = v_uv;
  vec3 fluidState = sampleFluidState(v_uv);
  uv += fluidState.rg * (0.038 + 0.18 * fluidState.b);
  uv = uv * 2.0 - 1.0;
  uv *= vec2(u_resolution.x / u_resolution.y, 1.0);

  float t = u_time * 0.2;
  vec2 movement = vec2(fbm(uv * 0.5 + t), fbm(uv * 0.5 + t + 2.0));
  float pattern = smoothstep(0.0, 1.0, fbm(uv + movement));

  vec3 c1 = vec3(0.02, 0.04, 0.06);
  vec3 c2 = vec3(0.05, 0.15, 0.25);
  vec3 c3 = vec3(0.18, 0.69, 0.81);
  gl_FragColor = vec4(mix(mix(c1, c2, pattern), c3, pattern * pattern), 1.0);
}
`;
...
export default function FluidBackground({ isActive = true }: FluidBackgroundProps) {
  ...
  useEffect(() => {
    ...
    const simW = Math.floor(w * 0.5), simH = Math.floor(h * 0.5);
    ...
    let fluidA = new THREE.WebGLRenderTarget(simW, simH, rtOpts);
    let fluidB = new THREE.WebGLRenderTarget(simW, simH, rtOpts);
    ...
    fluidScene.add(new THREE.Mesh(quadGeo, fluidMat));
    ...
    const animate = () => {
      ...
      fluidMat.uniforms.iMouse.value.set(
        mx.x * simW / w, (1 - mx.y / h) * simH,
        mx.px * simW / w, (1 - mx.py / h) * simH
      );
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      mouseTimerRef.current = setTimeout(() => {
        mouseRef.current = { x: 0, y: 0, px: 0, py: 0 };
      }, 100);
    };
    ...
    const onResize = () => {
      ...
      const nsw = Math.floor(nw * 0.5), nsh = Math.floor(nh * 0.5);
    ...
    return () => {
      ...
      if (mouseTimerRef.current) clearTimeout(mouseTimerRef.current);
      fluidA.dispose(); fluidB.dispose();
      fluidMat.dispose(); displayMat.dispose();
      quadGeo.dispose(); renderer.dispose();
